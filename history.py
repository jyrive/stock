...
def print_score_table(scores, title="Scores"):
    ...
    print(f"  {title}")
    print(HEADER)
    print(f"  {'#':<5}{'Symbol':<9}{'Name':<32}{'Score':>7}{'ROE%':>8}"
          f"{'EPS CAGR':>10}{'FCF Yld':>9}{'MoS%':>9}{'Price':>10}")
    print(f"  {DIVIDER}")

    for i, s in enumerate(scores, 1):
        name = (s.get("name") or "?")[:30]
        roe = f"{s['roe_pct']:.1f}%" if s.get("roe_pct") else "-"
        cagr = f"{s['eps_cagr']:.1f}%" if s.get("eps_cagr") else "-"
        fcf_y = f"{s['fcf_yield']:.1f}%" if s.get("fcf_yield") else "-"
        mos = f"{s['margin_of_safety']:.1f}%" if s.get("margin_of_safety") else "-"
        price = f"${s['current_price']:.2f}" if s.get("current_price") else "-"
        score = f"{s['buffett_score']:.1f}" if s.get("buffett_score") else "-"

        print(f"  {i:<5}{s['symbol']:<9}{name:<32}{score:>7}{roe:>8}"
              f"{cagr:>10}{fcf_y:>9}{mos:>9}{price:>10}")
...
def print_ticker_history(symbol, history):
    ...
    print(f"\n  {'Date':<14}{'Score':>8}{'EPS':>6}{'ROE':>6}{'FCF':>6}"
          f"{'ROE%':>8}{'CAGR':>9}{'FCF Yld':>9}{'Price':>10}{'MoS%':>9}")
    print(f"  {DIVIDER}")
    ...
    for row in history:
        ...
        roe_pct = f"{row['roe_pct']:.1f}%" if row.get("roe_pct") else "-"
        ...
        mos = f"{row['margin_of_safety']:.1f}%" if row.get("margin_of_safety") else "-"
        ...
        print(f"  {row['scan_date']:<14}{score_str:>8}{eps_s:>6}{roe_s:>6}{fcf_s:>6}"
              f"{roe_pct:>8}{cagr:>9}{fcf_y:>9}{price:>10}{mos:>9}")
