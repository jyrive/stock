...
def _convergence(fund, tech, macro):
    ...
    """
    pairs = {
        "F+T": _pair_converges(fund, tech),   # entry confirmed
        "F+M": _pair_converges(fund, macro),   # conviction confirmed
        "T+M": _pair_converges(tech, macro),   # timing confirmed
    }
...
def _raw_verdict(fund, tech, macro, conv_count):
    ...
    # 1 red present → WATCH or HOLD
    if reds == 1:
        if greens >= 2:
            return "WATCH", 0, 25
        return "HOLD", 0, 0

    # No reds — walk down the table
...
def compute_verdict(fund, tech, macro):
    ...
    # Veto rule: any score < 25 caps at WATCH
    veto = False
    for s in (fund, tech, macro):
    ...
    # Macro sizing multiplier adjusts the position band
