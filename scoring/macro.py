...
def _fetch_indicator(symbol, period="1y"):
    ...
    try:
        ...
        # 200-day and 50-day SMA (use available data)
        ...
        # YTD change (approximate — first trading day of year)
        ytd_pct = None
        if len(h) > 0:
            idx = h.index
            year_start = idx[0]
...
def _score_vix(vix_current):
    ...
    VIX > 35: 25, > 28: 20, > 22: 15, 18-22: 10, < 18: 5, < 13: 0
    """
...
def _score_yield_spread(spread):
    """Score 2Y-10Y yield spread: normal/steep = healthy. Inverted = danger.
    Inverted (< -0.5): 0, Flat (-0.5 to 0): 5, Mild (0-0.5): 12, Normal (0.5-1.5): 20, Steep (>1.5): 15
    """
    if spread is None:
        return 10
    if spread < -0.5:
        return 0   # deeply inverted — recession risk
    if spread < 0:
        return 5   # flat/slightly inverted
    if spread < 0.5:
        return 12  # mildly positive
    if spread < 1.5:
        return 20  # healthy normal curve
    return 15      # very steep — usually early recovery


def _score_sp500_52w(pos_pct):
    """Score S&P 500 52-week position: near lows = opportunity.
    < 20%: 15, < 35%: 12, < 50%: 8, < 70%: 5, < 85%: 3, else: 0
    """
    if pos_pct is None:
        return 7
    if pos_pct < 20:
...
def _score_10y_yield(yield_pct):
    ...
    < 2.5%: 15, < 3.5%: 13, < 4.0%: 10, < 4.5%: 7, < 5.0%: 4, else: 1
    """
