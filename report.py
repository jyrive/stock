...
def _build_report():
    ...
    if w_set:
        ...
        for sym in sorted(w_set):
            ...
            fund = latest_bs.get(sym, {}).get("buffett_score")
        ...
        if opps:
            ...
            for o in opps[:10]:
                ...
                lines.append(f"  {o['symbol']:<8} Score:{o['buffett_score']:>5.0f}  MoS:{o['margin_of_safety']:>6.1f}%  Price:{price}  IV:{iv}")
            ...
            for kind, items in p_alerts.items():
                for a in items:
                    if kind == "score_drops":
                        ...
                        lines.append(f"  ★ {a['symbol']} bargain — Score:{a.get('buffett_score', 0):.0f} MoS:{a.get('margin_of_safety', 0):.1f}%")
    ...
    lines.append("")
    lines.append("=" * 50)
    lines.append(f"Generated by Buffett Stock Screener  {now}")
    return "\n".join(lines)
